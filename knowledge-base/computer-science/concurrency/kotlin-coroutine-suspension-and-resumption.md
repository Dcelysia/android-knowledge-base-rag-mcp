...
> 本文是协程的深挖层：先重建同步、线程和回调方案，再沿一条可验证的 Android 链路解释协程如何启动、挂起、被外部事件持有、恢复和取消。只想复习进程、线程与协程的关系时，可阅读 [进程、线程与协程总览](/computer-science/concurrency/process-thread-coroutine#coroutine-overview)。
